...
#
# 1. 범주형 자료 ( categorical data) : 분류하고 그룹화.
#    - 범주 또는 그룹으로 구분할 수 있는 값
#    - 범주형 자료의 값은 기본적으로 숫자로 표현할 수 없고,
#      대소 비교나 산순연산이 적용되지 않는다.
#    - 범주형 자료는 대체로 문자형 값을 갖는데, 숫자로 표기할수 있으나
#      계산이 가능한 연속형 자료가 되는 것은 아니다.
#
# 2. 연속형 자료 ( numnerical data)
#    - 크기가 있는 숫자들로 구성된 자료료
...
#      하나의 변수로 구성된 자료
#   2. 다중변수 자료( multivariate data) , 다변량자료
#      두 개 이상의 변수로 구성된 자료
#      특별히 두 개의 변수로 구성된 자료를 이변량 자료( bivaritate data )
#
...
#
# 2.3 단일 변수 범주형 자료 탐색
#  ( 범주형 : 혈액형, 품종, 계절, 성별 etc, 범주형은 산술연산 x , table(빈도수, 함수 쓰기)
# 단일 변수 범주형 자료 : 특성이 하나이면서 자료의 특성이 범주형인 자료
#   범주형 자료에 할 수 있는 기본 작업 : 자료에 포함된 관측값들의 
#                          도수분포표 <- 종류별로 개수를 세는 것,                                      종류별로 비율을 구하는 것,
#                                        종류별로 비율을 구하는 것,
#                                        시각화는 막대/원 그래프를 사용
...
barplot( ds, main = 'favorite season') # 기본 막대그래프를 만들어주는 함수, ( ds, mian = 막대이름)

ds.new <- ds[ c(2, 3, 1, 4) ]
ds.new
barplot( ds.new, main = 'favorite season')  # 면 적  

pie( ds, main = 'favorite season')          # 전체적인 비율
...
barplot( ds, main = 'favorite season',            # 범주형 도수분포표
         col = colors)
...
#                   이상값에 영향을 받는다.
# 2. 중앙값( median ) : 자료의 값들을 크기순으로 정렬하였을때
#                       가장 중앙에 위치한 값,
#                       이상값에 영향을 받지 않는다. (홀수 일떄 1개, 짝수는 2개)
# 3. 절사평균 ( tirmmed mead) : 자료의 관측값들 중에서 작은 값들의 하위
#                               n%와 큰 값의 상위 n%를 제외하고 중간에있는
#                               나머지값들만 가지고 평균을 계산하는 방식 / 시험점수, 
# 4. 사분위수( quatrile) : 주어진 자료에 있는 값들을 크기순으로 나열
#                          했을 때 4등분하는 지점에 있는 값
#           1사분위수(Q1), 2사분위수(Q2, 중앙값과 동일) , 3사분위수(Q3)
#           전체 자료를 4개로 나누므로 4개 구간은 25%의 자료가 존재   / 범위밖은 이상치( 문제에 따라 절삭)
...
# -----> 평균값과 중앙값,절사평균값의 차이가 있을 때 평균값을 사용하기 어렵다.
...
# 연속형 자료 시각화 
# Histogram : 연속형 자료의 분포를 시각화 하는 도구
...
hist( dist, main = "Histogram for 제동거리",         # 연속형 도수 분포 / 구간의 빈도수를 구분할 때.
      xlab = "제동거리", ylab = "빈도수",            # 연속형 , 범주형 = 둘 다 빈도 수를 구함 but, 연속형은 구간, 범주형은넓이 
      border = 'blue', col = 'green',
      las = 2, breaks = 5)
 # 본질을 먼저 한 뒤, 나중에 그래프로 표현.


# 상자 그림( boxplot, 상자 수염 그래프)
#    사분위수를 그래프형태로 시각화하는 도구
#    상자그래프는 하나의 그래프로 데이이터의 분포 
#    형태를 포함한 다양한 정보를 전달 
#    - 자료의 전반적인 분포를 이해하는데 도움
#    - 구체적인 최소/최대/중앙값을 알기는 어렵다. / 구체적인 값을 알 때 : boxplot.stats or summary 함수 

boxplot( dist, main = '자동차 제동거리' ,
         col = 'black' , border = 'red') 

# 빅데이터는 이미 발생된 데이터를 이면을 분석하고 숨겨진 의미를 이해하여 우리 삶을 윤택하게 한다. 
...
boxplot( iris$Petal.Length~iris$Species,
         main = "품종별 꽃잎 길이",
         border = "red", col = "black")    # ( ~, 그룹정보가 있을 때 )
